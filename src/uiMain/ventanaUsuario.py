      ...
      def programa():
       ...
       # PARA VOLVER A LA VENTANA DE INICIO
         ...
         def eventoUsuario():
            ...
            frameInformacion = tk.Frame(mainFrame, bg="gray85", height=400, width=700) #gray82
         ...
         def eventoAplicacion():
            messagebox.showinfo(title = "Información de la Aplicación", message = "CONEXIA",
         ...
         def mejoraTuPlan():
            ...
            def eventoAceptar():
               ...
               try:
                  ...
                  clienteActual = proveedorActual.verificarCliente(documento, )
...
# ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
         ...
         def test_command():
            ...
            aceptar_button.pack( side="right", pady=20, padx=50, anchor="sw")
            borrar_button.pack( side="right", pady=20, padx=40, anchor="se")
         ...
         def func_test():
            ...
            self.frame_display=tk.Frame(frameInformacion, bg="grey85", height=400, width=500)
            self.frame_display.place(relx=0.5, rely=0.5, relheight=0.5,relwidth=0.5, anchor="center")
            ...
            if "saturada" in resultado:
               ...
               container = tk.Frame(self.frame_display)
               container.grid(row=1, column=0, padx=50, pady=10, sticky="w")

               canvas = tk.Canvas(container, width=300, height=200)
               ...
               # Agregar dispositivos a la lista desplazable
               dispositivos_cliente_lista = dispositivos_cliente.copy()

               def actualizar_lista():
                   for widget in scrollable_frame.winfo_children():
                       widget.destroy()
                   for i, dispositivo in enumerate(dispositivos_cliente_lista):
                       text_disps = tk.Label(scrollable_frame, text=f"{i+1}. {dispositivo}")
                       text_disps.pack(anchor="w", padx=10, pady=5)
               ...
               def eliminar_dispositivo():
                   try:
                       ...
                       if 0 <= index < len(dispositivos_cliente_lista):
                           dispositivo = dispositivos_cliente_lista.pop(index)
                           mensaje = Dispositivo.desconectarDispositivo(self.cliente_, dispositivo)
               ...
               # Frame para la entrada y botón a la derecha
               input_frame = tk.Frame(self.frame_display)
               input_frame.grid(row=1, column=1, padx=20, pady=10, sticky="n")

               entry = tk.Entry(input_frame)
               entry.pack(pady=5)

               delete_button = tk.Button(input_frame, text="Eliminar Dispositivo", command=eliminar_dispositivo)
               delete_button.pack()
               ...
               # Buscar antena compatible
               antena_compatible = antena.rastrearGeneracionCompatible(antenas_sede, self.cliente_.getModem())

               if antena_compatible:
                   # Si hay una antena compatible, mostrar opciones de cambio
                   ...
                   text_antenas = tk.Label(self.frame_display, bg="grey85", text=str(antena_compatible))
                   ...
                   # Si no hay antena compatible, mostrar mensaje de error
                   text_error = tk.Label(
                       self.frame_display, bg="grey85",
                       text="No se encontraron antenas compatibles \n en la zona. Contacte con soporte."
                   )
                   text_error.grid(row=0, column=1, padx=50, pady=20, sticky="w")

                   cerrar_button = tk.Button(self.frame_display, text="Cerrar", command=function2)
                   cerrar_button.grid(row=1, column=1, pady=10)
               ...
               # Buscar una antena compatible
               antena_compatible = antena.rastrearGeneracionCompatible(antenas_sede, self.cliente_.getModem())          
               if antena_compatible:
                   # Si hay antena accesible, mostrar opciones de cambio
                   ...
                   # Si no hay antenas accesibles, mostrar un mensaje diferente
                   text_error = tk.Label(
                       self.frame_display, bg="grey85",
                       text="No se encontraron antenas accesibles en tu zona.\n Contacta con soporte técnico para más información."
                   )
                   text_error.grid(row=0, column=0, padx=50, pady=20, sticky="w")            
                   cerrar_button = tk.Button(self.frame_display, text="Cerrar", command=function2)
                   cerrar_button.grid(row=1, column=0, pady=10)
